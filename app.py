...
def admin_item_edit(item_id):
    ...
    if request.method == 'POST':
        ...
        if request.form.get('delete_image') == '1' and item.image_url:
            img_path = os.path.join(app.config['UPLOAD_FOLDER'], item.image_url)
...
def admin_meat_list():
    ...
    return render_template('admin/meat_list.html', meat_options=meats)
...
def admin_spicy_list():
    ...
    levels = SpicyLevel.query.order_by(SpicyLevel.level).all()
    return render_template('admin/spicy_list.html', spicy_levels=levels)
...
def admin_spicy_new():
    ...
    if request.method == 'POST':
        ...
        existing = SpicyLevel.query.filter_by(level=level_val).first()
        if existing:
            flash(f'Level {level_val} already exists ({existing.label}). Please choose a different number.', 'danger')
            return redirect(url_for('admin_spicy_new'))
...
def admin_spicy_edit(spicy_id):
    ...
    if request.method == 'POST':
        ...
        existing = SpicyLevel.query.filter_by(level=spicy.level).first()
        if existing and existing.id != spicy.id:
            flash(f'Level {spicy.level} already exists ({existing.label}). Please choose a different number.', 'danger')
            return render_template('admin/spicy_form.html', spicy=spicy)
